...
# Stores error that not a Voicelink Exception
ERROR_LOGS: dict[int, dict[int, str]] = {}
...
def langs_setup() -> None:
    ...
    for language in os.listdir(os.path.join(ROOT_DIR, "local_langs")):
        if language.endswith('.json'):
            LOCAL_LANGS[language[:-5]
                        ] = open_json(os.path.join("local_langs", language))
...
async def send(ctx: Union[commands.Context, discord.Interaction], key: str, *params, delete_after: float = None, ephemeral: bool = False) -> Optional[discord.Message]:
    ...
    send_func = ctx.send if isinstance(ctx, commands.Context) else (
        ...
        for key, value in action.items():
            ...
            if mode == "$set":
                ...
                nested_data[cursors[-1]
                            ] = nested_data.get(cursors[-1], 0) + value

            elif mode == "$push":
                if isinstance(value, dict) and "$each" in value:
                    nested_data.setdefault(
                        cursors[-1], []).extend(value["$each"][value.get("$slice", len(value["$each"])):])
                else:
                ...
                if cursors[-1] in nested_data:
                    value = value.get("$in", []) if isinstance(
