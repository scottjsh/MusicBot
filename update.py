...
GITHUB_API_URL = "https://api.github.com/repos/scottjsh/MusicBot/releases/latest"
VOCARD_URL = "https://github.com/scottjsh/MusicBot/archive/"
...
def download_file(version=None):
    ...
    print(f"Downloading BSG version: {version}")
...
def install(response, version):
    ...
    if user_input.lower() in ["y", "yes"]:
        ...
        source_dir = os.path.join(ROOT_DIR, f"BSG-{version}")
        if os.path.exists(source_dir):
            for filename in os.listdir(ROOT_DIR):
                if filename in IGNORE_FILES + [f"BSG-{version}"]:
                    continue
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Update script for BSG.')
    parser.add_argument('-c', '--check', action='store_true', help='Check the current version of the BSG')
    parser.add_argument('-v', '--version', type=str, help='Install the specified version of the BSG')
    parser.add_argument('-l', '--latest', action='store_true', help='Install the latest version of the BSG from Github')
    parser.add_argument('-b', '--beta', action='store_true', help='Install the beta version of the BSG from Github')
