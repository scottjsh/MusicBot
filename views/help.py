...
class HelpDropdown(discord.ui.Select):
    def __init__(self, categorys: list):
        ...
        super().__init__(
            ...
            options=[
                discord.SelectOption(emoji="🆕", label="정보",
                                     description="백설기의 새로운 업데이트 보기."),
            ] + [
                discord.SelectOption(
                    emoji=emoji, label=f"{category} 명령어", description=f"이것은 {category.lower()} 카테고리입니다.")
...
class HelpView(discord.ui.View):
    def __init__(self, bot: commands.Bot, author: discord.Member) -> None:
        ...
        self.categorys: list[str] = [name.capitalize(
        ) for name, cog in bot.cogs.items() if len([c for c in cog.walk_commands()])]

        self.add_item(discord.ui.Button(label='초대', emoji=':invite:915152589056790589',
                      url=f'https://discord.com/oauth2/authorize?client_id={bot.user.id}&permissions=8&scope=bot%20applications.commands'))
        self.add_item(discord.ui.Button(
            label='Github', emoji=':github:1098265017268322406', url='https://github.com/scottjsh/MusicBot'))
    ...
    def build_embed(self, category: str) -> discord.Embed:
        ...
        if category == "news":
            embed = discord.Embed(title="백설기 도움말 메뉴",
                                  color=func.settings.embed_color)
            embed.add_field(
                name=f"카테고리: [{1 + len(self.categorys)}]",
                value="```py\n👉 정보\n{}```".format(
            ...
            embed.add_field(
                name="시작하기", value="```음성 채널에 참여한 후 /play {노래/URL}로 노래를 재생하세요. (이름, 유튜브 비디오 링크, 재생 목록 링크 또는 스포티파이 링크가 지원됩니다.)```", inline=False)

            return embed

        embed = discord.Embed(
            title=f"카테고리: {category.capitalize()}", color=func.settings.embed_color)
        embed.add_field(name=f"카테고리: [{1 + len(self.categorys)}]", value="```py\n" + "\n".join(("👉 " if c == category.capitalize(
        ) else f"{i}. ") + c for i, c in enumerate(['정보'] + self.categorys, start=1)) + "```", inline=True)
        ...
        embed.add_field(
            name=f"{category} 명령어: [{len(commands)}]",
            value="```{}```".format("".join(
